...
1. Uninstall and reinstall the ssh server using the following commands:
    ...
    3.  Edit the `/etc/ssh/sshd_config` file by running the command `sudo nano /etc/ssh/sshd_config`
    4.  Change `Port` to 2222 (or any other port above 1000)
...
4. If configured properly, the ssh server should now automatically start in the background when Windows starts.
    1. Check your fire wall to allow sshd connections
    2. Check your 'Power Settings' iot turn off 'Sleeping' 
